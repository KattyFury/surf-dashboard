...
**Ý nghĩa thực tế khi build**: 30 credit/ngày đủ cho vài chục lệnh `--help`/thử nghiệm hoặc 1 dashboard nhẹ (kiểu `fetch-content.mjs`, ~10-15 call). Nhưng nếu ý tưởng của bạn cần quét **hàng trăm** dự án qua `/project/detail` (ví dụ để so sánh toàn thị trường), hãy tự ước lượng số credit **trước khi chạy thật** (số lần gọi × chi phí mỗi loại call ở trên) — free tier hết sạch chỉ sau vài chục request đầu, phần còn lại sẽ lỗi 429/hết quota, và nếu key đã top-up thì bạn trả tiền thật cho mỗi request. Giữ cache + concurrency thấp (xem Bước 4) để không đốt credit vô ích khi chạy thử đi thử lại.
...
Không có công thức chung ở bước này — tuỳ bạn muốn kể câu chuyện gì mỗi tuần. Ví dụ thật mình đã chạy, để tham khảo cách gọi API + xử lý dữ liệu, **không phải để bạn copy y nguyên**:

- [`fetch-content.mjs`](fetch-content.mjs) — gọi 3 nhóm endpoint (Token of the Week, watchlist vài quỹ VC cố định qua `/fund/portfolio`, fundraising importance cao qua `/search/fundraising`), ghi ra 1 file JSON. Nhẹ, nằm gọn trong free tier 30 credit/ngày.

Node script thuần (`node fetch-content.mjs`), đọc API key từ file `.env.txt` (không commit — xem `.gitignore`), không có backend/cron gì phức tạp. Chạy thủ công khi nào muốn cập nhật số liệu để đăng bài.
